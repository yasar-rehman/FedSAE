...
class Server(BaseFedarated):
    ...
    def train(self):
        ...
        for i in range(self.num_rounds):
            ...
            for idx, c in enumerate(selected_clients.tolist()):
                ...
                if(c.ideal_epoch >= difficult):                  #client能完成difficult的任务
                    ...
                    if(c.bit_epoch > 0):
                      easy = min(bit_epoch, difficult+increament)
                      difficult = max(bit_epoch, difficult+increament)
        ...
        #self.write_data_to_file(trainer="fedsae", str_dataset="mnist", distribution="normal_σ=ran(0.25μ_0.5μ)", file_name="scheme3_bit_epoch", test_acc=test_acc, train_acc=train_acc, loss=loss, drop_out_percentage = drop_out_percentage)
        #self.get_result(trainer="fedsae", str_dataset="mnist", distribution="normal_σ=ran(0.25μ_0.5μ)", file_name="scheme3_bit_epoch")
