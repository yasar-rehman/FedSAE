...
# 这个版本加了对variance的计算
# fedavg_no_dropput忽略客户端的drop out即使客户端drop out了也能将自己此时的训练结果上传给客户端
    ...
    def train(self):
        ...
        for i in range(self.num_rounds):                                                 #num_rounds也是传进来的参数
            ...
            if i % self.eval_every == 0:   
                ...
                print("np.sum(_stats[3]) * 1.0", np.sum(_stats[3]) * 1.0)
                print("np.sum(_stats[2])", np.sum(_stats[2]))
            ...
            # 如果客户端实际能完成的任务超过服务器，也只能完成服务器分配的任务量
            # 只是客户端实际能完成的任务量小于服务器任务量
            for s_c in selected_clients:
                ...
                if(s_c.get_ideal_epoch() > actual_epoch):
                    s_c.set_ideal_epoch(actual_epoch)        
            ...
            for c in active_clients:
                ...
                iters = int((c.ideal_epoch - int(c.ideal_epoch)) * total_iters)             #计算epoch小数部分，转换成iteration,然后计算epoch的整数部分
                #iters = int((self.num_epochs - int(self.num_epochs)) * total_iters)  
                if(iters > 0):
        ...
        print("lowest accuracy vaviance: ", acc_variance)
        ...
        self.write_data_to_file0(trainer="fedavg_no_dropout", str_dataset="fmnist", distribution="normal_σ=ran(0.25μ_0.5μ)", file_name="fmnist_fedavg_nodrop_epoch15_c10_300r_mclr", test_acc=test_acc, train_acc=train_acc, loss=loss, drop_out_percentage = drop_out_percentage, test_acc_variance=test_acc_variance, train_acc_variance=train_acc_variance)
        self.get_result0(trainer="fedavg_no_dropout", str_dataset="fmnist", distribution="normal_σ=ran(0.25μ_0.5μ)", file_name="fmnist_fedavg_nodrop_epoch15_c10_300r_mclr")
