    ...
    def train(self):
        ...
        for i in range(self.num_rounds):
            ...
            if i % self.eval_every == 0:
                ...
                #if(round == 0): self.update_loss(self.clients)
            ...
            # self.select_clients 普通选择, self.hierarchical_select_clients 先聚类后loss选择
            indices, selected_clients = self.hierarchical_select_clients(i, num_clients=self.clients_per_round)  # uniform sampling
            ...
            for idx, c in enumerate(selected_clients.tolist()):
                ...
                #使用标准TCP算法的融合版(highest_acc=0.7442827,drop=0.0,hit=0.05)
                # [easy, difficult]
                #    √       √            [easy+10/easy, difficult+10/difficult](均选较小的在左，较大的在右)
                #    √       ×            [easy+10/easy, difficlult/2]
                #    ×       ×            [easy/2, difficult/2]
                '''
                ...
                if(c.ideal_epoch >= difficult):                  #client能完成difficult的任务
                    ...
                    if(c.bit_epoch > 0):
                      easy = min(easy+10.0/easy, difficult+10.0/difficult)  #+10.0/(easy*easy),+10.0/(difficult*difficult)
                      difficult = max(easy+10.0/easy, difficult+10.0/difficult)
                ...
                #print("debug1: ideal_epoch: {}, bit_epoch: {}, r0: {}, r1: {}, [easy, difficult]: [{}, {}]".format(c.ideal_epoch, bit_epoch, r0, r1, easy, difficult))
                ...
                c.set_actual_epoch(actual_epoch)
                if(segment > 0):   # 分段参数segment不为-1则使用分段上传
                ...
                if(segment > 0):
                  for s in soln:
                    csolns.append(s)             #segment>0时soln中存放的是几个段上传的数据，需要把数据从列表中取出来再放到csolns中  
                else:
                  csolns.append(soln)
        ...
        self.write_data_to_file(trainer="fedsae_correctblosstcp", str_dataset="femnist", distribution="normal_σ=ran(0.25μ_0.5μ)", file_name="femnist_fedsae_HCtcp_hc_predict_num_class3_epoch15_c10_200r_CNN", test_acc=test_acc, train_acc=train_acc, loss=loss, drop_out_percentage = drop_out_percentage, selected_times=selected_times, test_acc_variance=test_acc_variance, train_acc_variance=train_acc_variance)
        self.get_result(trainer="fedsae_correctblosstcp", str_dataset="femnist", distribution="normal_σ=ran(0.25μ_0.5μ)", file_name="femnist_fedsae_HCtcp_hc_predict_num_class3_epoch15_c10_200r_CNN")
        ...
        for i in range(client_range):
            ...
            if(cmiu[i]-csigma[i]<c.two_task[0] and cmiu[i]+csigma[i]>c.two_task[1]): hit += 1
