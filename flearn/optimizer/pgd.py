...
class PerturbedGradientDescent(optimizer.Optimizer):                                  #继承优化器类，实现一个优化器？扰动梯度下降是什么
    ...
    def _prepare(self):                                                               #单斜杠前线，受保护的函数，只能类和子类调用
        self._lr_t = ops.convert_to_tensor(self._lr, name="learning_rate")
        self._mu_t = ops.convert_to_tensor(self._mu, name="prox_mu")

    def _create_slots(self, var_list):
        # Create slots for the global solution.                                       #slot为槽，限制类能定义的属性
        for v in var_list:
            self._zeros_slot(v, "vstar", self._name)                                  #父类Optimizer的函数_zeros_slot找到或创造一个初始化为0的槽

    def _apply_dense(self, grad, var):
        lr_t = math_ops.cast(self._lr_t, var.dtype.base_dtype)                        #cast()将一个tensor转换成其他类型
